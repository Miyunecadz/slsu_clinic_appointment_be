...
const getTodaySchedules = async (req, res) => {
    ...
    var timeToday = hours + ":" + minutes;

    const dateToday = year + "-" + month + "-" + day;
    const scheds = await Schedule.showMany({deleted_flag: false})
    const schedules = scheds.filter((sched) => sched.date >= dateToday && sched.time >= timeToday )
    return res.json({
        'result': true,
        'schedules': schedules
    })
